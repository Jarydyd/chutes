...
def test_ensure_chutes_dir_creates_secure_directory_and_gitignore(manage_credentials_module):
    ...
    # Windows often reports 0o777 for user dirs; chmod bits are not enforced like Unix.
    if os.name != "nt":
...
def test_derive_key_is_stable_32_bytes(manage_credentials_module):
    module = manage_credentials_module
    k1 = module._derive_key()
    k2 = module._derive_key()
    assert len(k1) == 32
    assert k1 == k2


def test_platform_user_identity_shape(manage_credentials_module, monkeypatch):
    """POSIX uses numeric uid; Windows uses win:DOMAIN:USER (getuid is absent)."""
    module = manage_credentials_module
    if os.name == "nt":
        monkeypatch.setenv("USERNAME", "alice")
        monkeypatch.setenv("USERDOMAIN", "CORP")
        assert module._platform_user_identity() == "win:CORP:alice"
    else:
        assert module._platform_user_identity().isdigit()
